...
const calculateDistance = (gameLocation, guessLocation) => {
  ...
  let distance =
    Math.sin(radGameLat) * Math.sin(radGuessLat) +
    Math.cos(radGameLat) * Math.cos(radGuessLat) * Math.cos(radTheta);
  if (distance > 1) {
    distance = 1;
  }
...
const generateDailyGameNum = () => {
  const startDate = new Date(2023, 6, 19); // month is by index (so april = 3), setting startDay to today will result in id = 1;
